...
bigquery/
├─ raw/
│   ├─ github_events        # コミット、PR、Issue の生イベント
│   ├─ linear_events        # Linear の状態変更イベント
│   └─ communication_summaries  # Slack/Discord の要約（生データではない）
│
├─ enriched/
│   ├─ project_outcomes     # 見積 vs 実績の構造化比較
│   ├─ team_velocity        # チーム単位の Velocity 時系列
│   ├─ evidence_quality     # プロバイダ別エビデンス精度
│   └─ customer_signals     # 顧客コミュニケーションのシグナル
│
└─ analytics/
    ├─ estimation_accuracy  # 見積精度レポート
    ├─ customer_portfolio   # 顧客ポートフォリオ分析
    ├─ capacity_forecast    # キャパシティ予測
    └─ pricing_trends       # 価格トレンド分析
```

全テーブルに `tenant_id` を持たせ、パーティションキーとする。
...
テナントごとに独立した学習ループが回る構造は、SaaS の価値提案として強い。
