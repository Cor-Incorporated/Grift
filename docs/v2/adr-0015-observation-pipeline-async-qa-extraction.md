...
> **TODO** (Issue #147): 将来の ADR で、既存 PascalCase イベントから dot.notation への段階的マイグレーションパスを formalize すること。マイグレーション期間中は両形式のイベントが共存するため、コンシューマー側でのイベントタイプ解決ロジックの設計が必要になる。
...
### 6.5. Completeness feedback の統合パターン

`control-api` が各ターンの System Prompt を生成する直前に、Cloud SQL から最新 completeness 状態を直接読み取る方式（DB ポーリング）を採用する。

採用方式（Primary）:

1. `control-api` が `conversation_turns` 保存後に最新 `overall_completeness`, `missing_items`, `updated_at` を参照
2. System Prompt 末尾へ `未収集項目: [項目1, 項目2, ...]` を注入
3. `overall_completeness >= 0.8` の場合、conversation に `is_complete: true` を設定

評価した代替方式（Secondary）:

- `control-api` が `observation.completeness.updated` を Pub/Sub サブスクライブしてローカルキャッシュを更新する方式

代替方式を採用しない理由:

- 会話パスの read-after-write 一貫性を担保しづらい（eventual consistency の遅延で古い completeness を参照する）
- control-api 側に常駐 subscriber の運用責務が増える
- Polls 教訓に反して、会話制御の依存点が増える

補足:

- DB ポーリング方式でも eventual consistency は許容する。最新行が未反映のときは直前値で継続し、次ターンで収束させる
- Pub/Sub 方式は将来の高スループット要件で再評価可能とする
