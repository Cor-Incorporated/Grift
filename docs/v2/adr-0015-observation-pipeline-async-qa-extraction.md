...
GCP Pub/Sub は ordering key によるテナント分離とメッセージスキーマ検証（Avro / Protocol Buffers）をサポートする。at-least-once delivery と dead letter topic により、抽出失敗メッセージの安全なリトライが可能。（コンシューマー側の冪等処理は ADR-0009 で規定）
...
注: 上記イベント例では `aggregate_version` と `causation_id` を省略している。`aggregate_version` は conversation_turns テーブルが append-only であるためバージョン競合が発生しない。`causation_id` は Pipeline 内部で自動設定される（入力イベントの `event_id` を後続イベントの `causation_id` に伝播する）。実装時は ADR-0009 の正規エンベロープに完全準拠すること。
...
class Extractor(Protocol):
    def domain(self) -> str: ...
    def extract(self, turn: ConversationTurn) -> list[QAPair]: ...
...
GCP Pub/Sub は ordering key によるテナント分離、at-least-once delivery、dead letter topic をサポートする。スキーマ検証（Avro / Protocol Buffers）により、イベントフォーマットの不整合を publish 時に検出できる。
